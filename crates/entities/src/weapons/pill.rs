...
use crate::characters::enemy::{Enemy, EnemyType};
...
fn inject_rotating_pill_sprite(
    ...
    q_rotating_pills: Query<(&ChildOf, &mut Transform), With<RotatingPill>>,
    q_enemies: Query<(Entity, &ChildOf, &Transform, &Enemy), Without<RotatingPill>>,
...
fn rotate_rotating_pills(
    mut q_rotating_pills: Query<(&ChildOf, &mut Transform), With<RotatingPill>>,
    time: Res<Time>,
) {
    let elsapsed_seconds = time.elapsed_secs() * 3.0;
...
fn handle_pill_throw_event(
    ...
    q_enemies: Query<(Entity, &ChildOf, &Transform, &Enemy)>,
    mut ev_pill_throw: MessageReader<PillThrowEvent>,
    static_sprite_assets: Res<StaticSpriteAssets>,
) {
    for pill_throw_event in ev_pill_throw.read() {
        let (pill_initial_position, parent) = q_enemies
            .get(pill_throw_event.entity)
            .map(|(_, parent, transform, _)| (transform.translation, parent.parent()))
            .unwrap();
        ...
        let throw_vector = (player_position - pill_initial_position.normalize()).truncate();
        ...
        commands.entity(parent).with_children(|parent| {
            ...
            parent.spawn((
                PillBundle {
                    ...
                    transform: Transform::from_translation(pill_initial_position),
...
fn handle_pill_throw(
    ...
    mut q_pill: Query<(Entity, &ChildOf, &RotatingPill, &mut PillTimer)>,
    mut ev_arrow_attack: MessageWriter<PillThrowEvent>,
    time: Res<Time>,
) {
    for (_, parent, _, mut timer) in q_pill.iter_mut() {
        timer.0.tick(time.delta());
        if timer.0.just_finished() {
            let enemy_entity = q_enemies.get(parent.parent()).unwrap().0;
            ev_arrow_attack.write(PillThrowEvent {
                entity: enemy_entity,
            });
        }
    }
...
impl Plugin for WeaponPillPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<PillTimer>()
            .add_message::<PillThrowEvent>()
            .add_systems(
                Update,
                (
                    inject_rotating_pill_sprite.run_if(in_state(GameState::GamePlay)),
                    rotate_rotating_pills.run_if(in_state(GameState::GamePlay)),
                    handle_pill_throw.run_if(in_state(GameState::GamePlay)),
                    handle_pill_throw_event.run_if(in_state(GameState::GamePlay)),
                    handle_arrow_timers.run_if(in_state(GameState::GamePlay)),
                    handle_trail_timers.run_if(in_state(GameState::GamePlay)),
                    handle_pill_player_collisions.run_if(in_state(GameState::GamePlay)),
                ),
            );
    }
}
