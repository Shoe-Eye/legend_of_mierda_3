...
pub fn event_on_biboran_step_over(
    ...
    for e in er_item_step_over.read() {
        ...
        for (e_biboran, item) in q_items.iter_mut() {
            ...
            commands.entity(e_biboran).despawn();
        }
    }
}
...
fn biboran_holy_effect(
    ...
    if !biboran_timer.0.just_finished() && biboran_effect_timer.0.just_finished() {
...
fn ineject_biboran_render_sprite(
    mut commands: Commands,
    q_players: Query<(&ChildOf, &Transform, &Player)>,
    ...
    for (parent, player_transform, _) in q_players.iter() {
        if q_biboran_sprite.iter().count() == 0 {
            commands.entity(parent.parent()).with_children(|parent| {
                parent.spawn((
                    Sprite {
                        image: biboran_render_image.0.clone(),
                        ..default()
                    },
                    Visibility::Hidden,
...
impl Plugin for BiboranPlugin {
    fn build(&self, app: &mut App) {
        app.register_ldtk_entity::<BiboranBundle>("Biboran")
            ...
            .add_systems(Update, event_on_biboran_step_over)
            .add_systems(Update, ineject_biboran_render_sprite);
    }
}
