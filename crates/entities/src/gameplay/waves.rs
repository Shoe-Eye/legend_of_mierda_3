    ...
    pub fn select_random_wave_entry(&mut self) -> Option<WaveEntry> {
        match self.current_wave() {
            Some(wave) => {
                ...
                let random_index = rng.random_range(0..self.event_queue.len());
...
pub fn event_on_level_change(
    mut er_on_level_change: MessageReader<LevelChangeEvent>,
    mut gameplay_state: ResMut<GameplayState>,
...
pub fn event_wave(
    mut er_on_wave_change: MessageReader<WaveEvent>,
